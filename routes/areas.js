...
router.post('/', upload.array('images', 10), async (req, res) => {
  try {
    ...
    if (!area) {
      area = new Area({
        areaname,
        categories: [{ category, shops: [{ shopname, products: [{ ...products[0], image: imageFiles[0].path }] }] }]
      });
    } else {
      let categoryObj = findCategory(area, category);

      if (!categoryObj) {
        area.categories.push({ category, shops: [{ shopname, products: [{ ...products[0], image: imageFiles[0].path }] }] });
      } else {
        ...
        if (shop) {
          ...
          categoryObj.shops.push({ shopname, products: [{ ...products[0], image: imageFiles[0].path }] });
        }
      }
    }
    await area.save();
...
router.get('/:areaname/:category/:shopname/:productname?', async (req, res) => {
  ...
  try {
    ...
    if (productname) {
      const product = shop.products.find(product => product.productname === productname);
      if (product) {
